...
function play() {
    ...
    function create_pipe() {
        ...
        if (pipe_seperation > 115) {
            ...
            let pipe_sprite = document.createElement('pipe');
    ...
    function move() {
        if (game_state != 'Play') return;
        let pipe_sprite = document.querySelectorAll('.piper_sprite');
        pipe_sprite.array.forEach(element => {
    ...
    function apply_gravity() {
        ...
        if (bingus_props.top <= 0 ||
            bingus_props.bottom >= background.bottom) {
            game_state = 'End';
            message.textContent = 'Press Enter to Restart';
            message.style.left = '28vw';
            return;
        }
        bird_style.top = bingus_props.top + bird_dy + 'px';
        bird_dy.props = bingus.getBoundingClientRect();
